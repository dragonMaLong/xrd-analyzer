...
- **单样品工程**：每个 `.xrdproj` 只保存一个样品的一套参数和曲线；只有显式导入工程文件才恢复计算结果
...
「导入文件」按钮同时支持 TXT、RAW 和 `.xrdproj` 工程文件。每个工程文件只对应
一个样品，内部包含原始 X/Y 数据、峰位与背景设置、计算参数、Rfit 历史以及
最后一次成功计算结果；导入后无需重新拟合即可恢复各个曲线。

在样品列表中右键单个样品，可选择「删除」「保存工程」或「另存为工程」。按住
Ctrl 可逐个多选，按住 Shift 可连续选择，也可按住鼠标左键拖动选择；多选后
右键仅提供「删除」。这里的删除只会将样品移出当前列表，不会删除磁盘上的原始
数据或工程文件。

TXT、RAW 和工程文件均在后台读取，工程压缩也在后台执行，期间界面仍可继续
操作。右侧标签栏的绿色进度条会显示当前正在解析、校验、读取或压缩的数据阶段，
完成后自动隐藏。如果保存过程中又修改了参数，新修改会继续保持为“未保存”状态。

直接导入 TXT/RAW 时始终只读取原始测量数据，并作为新的待分析样品打开，
不会根据数据内容指纹自动套用任何历史参数或结果。同一份原始数据可以保存为
多个采用不同算法或参数的 `.xrdproj`；需要恢复哪套分析时，应显式导入对应工程文件。
